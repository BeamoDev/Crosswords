...
DuelBots randomizes the 1v1 lobby population from two bots through the available server seats (minimum clamped to capacity). With a server size of 12, a lone player can see 2-11 bots, two humans can see 2-10 bots plus each other, and a full human server sees no bot list entries. The target rerolls on real-player count changes and every 60-120 seconds, rather than on every four-second UI refresh. Real players appear first. Bots use stable session identities and verified Roblox headshots; unready thumbnails stay out of the list. The existing 30-colour mugshot palette and darker strokes apply to bot rows too.
